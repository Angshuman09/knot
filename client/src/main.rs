...
fn print_usage() {
    eprintln!(
        "usage:\n\
         ...
         \n\
         example:\n\
         \x20 client 127.0.0.1:9000 deposit alice 100.00\n\
...
fn parse_dollars(s: &str) -> Result<Cents, String> {
    ...
    let frac_cents: i64 = match frac.len() {
        ...
        2 => frac
            .parse::<i64>()
            .map_err(|_| format!("not a number: {s}"))?,
        _ => return Err(format!("too many decimal places")),
    };
...
async fn main() -> std::io::Result<()> {
    ...
    let request = match command {
        "deposit" | "withdraw" => {
            let (Some(account), Some(amount_str)) = (args.get(3), args.get(4)) else {
                print_usage();
                return Ok(());
            };
        ...
        "transfer" => {
            let (Some(from), Some(to), Some(amount_str)) = (args.get(3), args.get(4), args.get(5))
            else {
                print_usage();
                return Ok(());
            };
