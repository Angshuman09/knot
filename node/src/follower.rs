...
impl Follower {
    pub fn open(data_dir: &str, leader_client_addr: String) -> std::io::Result<Self> {
        let ledger = Ledger::open(data_dir)?;
        println!(
            "follower: loaded {} entries from WAL ({data_dir})",
        ...
        loop {
            ...
            match message {
                ReplicationMessage::Entry(entry) => {
                    ...
                    let _ = ledger.append_replicated(entry);
                }
...
impl RequestHandler for Follower {
    async fn handle_request(&self, request: ClientRequest) -> ClientResponse {
        match request {
            ...
            | ClientRequest::Transfer { .. } => self.reject_write(),
        }
    }
}
